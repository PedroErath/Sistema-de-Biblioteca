    ...
    public class Autenticacao
    {
        ...
        public static bool verificaLoginSenha(string Login, string Senha, Controller controller)
        {
            using (BibliotecaContext bc = new BibliotecaContext())
            {
                ...
                List<Usuario>ListauserEncontrado = userEncontrado.ToList();

                if(ListauserEncontrado.Count==0)
                {
                    return false;
                }
                else
                {
                    controller.HttpContext.Session.SetString("Login",ListauserEncontrado[0].Login);
                    controller.HttpContext.Session.SetString("Nome",ListauserEncontrado[0].Nome);
                    controller.HttpContext.Session.SetInt32("Tipo",ListauserEncontrado[0].Tipo);
                    return true;
                }
            }
        }
